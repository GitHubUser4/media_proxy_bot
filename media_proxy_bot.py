...
def split_large_video(video_path, output_dir, max_parts=3):
    ...
    if parts_needed > max_parts:
        logger.warning(
            f"Видео слишком тяжелое: {file_size_mb:.2f} MB требует {parts_needed} частей."
        )
        return None
    ...
    try:
        ...
        duration = float(
            subprocess.check_output(cmd_duration, shell=True).decode().strip()
        )
            ...
            try:
                ...
                for _ in range(12):
                    ...
                    for el in elements:
                        ...
                        if not box or box["width"] < 300 or box["y"] > 700:
                            continue
                        ...
                        if src and src not in all_urls:
                            ...
                            ext = (
                                "mp4"
                                if await el.evaluate("n => n.tagName") == "VIDEO"
                                else "jpg"
                            )
                            f_path = os.path.join(
                                temp_dir, f"file_{len(all_urls)}.{ext}"
                            )
                            if await safe_download(src, f_path):
                                saved_files.append(f_path)
...
async def handle_youtube_mp3(message: Message):
    ...
    if len(parts) < 2:
        await message.answer(
            "⚠️ Отправь ссылку: `/mp3 https://youtu.be/...`", parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        t_dir = os.path.join(
            TEMP_BASE_DIR, f"yt_{message.from_user.id}_{int(time.time())}"
        )
...
async def cmd_status(msg: Message):
    ...
    ig_time = (
        datetime.fromtimestamp(os.path.getmtime(IG_COOKIE_FILE)).strftime("%d.%m %H:%M")
        if os.path.exists(IG_COOKIE_FILE)
        else "None"
    )
    yt_time = (
        datetime.fromtimestamp(os.path.getmtime(YT_COOKIE_FILE)).strftime("%d.%m %H:%M")
        if os.path.exists(YT_COOKIE_FILE)
        else "None"
    )
    await msg.answer(
        f"<b>📊 Статус:</b>\nRAM: {ram.percent}%\nDisk free: {disk.free // 10**9}GB\nInsta_cookies: {ig_time}\nYoutube_cookies: {yt_time}",
        parse_mode="HTML",
    )
    ...
    try:
        ...
        try:
            ...
            info = await loop.run_in_executor(
                ...
                lambda: YoutubeDL(
                    {
                        "outtmpl": f"{t_dir}/%(id)s.%(ext)s",
                        "quiet": True,
                        "cookiefile": IG_COOKIE_FILE,
                    }
                ).extract_info(clean_url, download=True),
            )
        ...
        if not files:
            raise Exception("Контент не найден.")
        ...
        for f in files:
            ...
            if f.lower().endswith((".mp4", ".mov")):
                ...
                if parts:
                    ...
                    logger.warning(f"Файл {f} пропущен, так как не может быть порезан.")
            # Если это картинка — проверяем лимит
            else:
                if os.path.getsize(f) <= MAX_SIZE_BYTES:
                    final_media.append(f)

        if not final_media:
            raise Exception("Файлы слишком тяжелые для TG.")
                        ...
                        if len(chunk) == 1:
                            ...
                            if f.endswith(".mp4"):
                                ...
                                await msg.answer_photo(
                                    FSInputFile(f), caption=current_caption
                                )
                        else:
